...
/*   Updated: 2019/06/05 16:50:59 by tle-dieu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	more_conv(va_list arg, t_print *buff, t_flag *flags, char **format)
{
	if (**format == 'b')
		oux_conv(buff, flags, get_unsigned(arg, flags), "01\0");
	else if (**format == 'C' || **format == 'c')
		lc_conv(buff, flags, format, va_arg(arg, wint_t));
	else if (**format == 'S' || **format == 's')
		ls_conv(buff, flags, format, va_arg(arg, wchar_t *));
	else if (flags->width)
...
static int	get_conv(va_list arg, t_print *buff, t_flag *flags, char **format)
{
	if (**format == 's' && !flags->l)
		s_conv(buff, flags, va_arg(arg, char *));
	else if (**format == 'd' || **format == 'i' || **format == 'D')
		di_conv(buff, flags, get_signed(arg, flags));
	else if (**format == 'c' && !flags->l)
		c_conv(buff, flags, va_arg(arg, int));
	else if (**format == '%')
		c_conv(buff, flags, '%');
	else if (**format == 'u' || **format == 'U')
		oux_conv(buff, flags, get_unsigned(arg, flags), "0123456789\0");
	else if (**format == 'x')
		...
		f_conv(arg, buff, flags, **format);
	else
		return (more_conv(arg, buff, flags, format));
	return (1);
}
...
static void	apply_format(va_list arg, t_print *buff, char const *format)
{
	int		color;
	t_flag	flags;
	...
	while (*format)
	{
		...
		{
			while (*++format && !get_flags(arg, &flags, (char **)&format))
				;
			if (get_conv(arg, buff, &flags, (char **)&format))
				format++;
		}
		else if (!color)
		{
